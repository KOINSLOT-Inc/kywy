...
def load_and_prepare_image(input_path, target_width=KYWY_WIDTH, target_height=KYWY_HEIGHT, 
                          threshold=None, resize=True):
    """
    ...
    try:
        ...
        if resize and img.size != (target_width, target_height):
            print(f"Resizing from {img.size[0]}x{img.size[1]} to {target_width}x{target_height}...")
            ...
            final_img = Image.new('L', (target_width, target_height), 255)  # White background
        ...
        if threshold is not None:
            print(f"Converting to 1-bit black/white with custom threshold {threshold}...")
...
def extract_sprite_frames(img, cols, rows):
    ...
    print(f"Extracting {cols}x{rows} sprite frames ({frame_width}x{frame_height} each)...")
...
def generate_header_file(byte_arrays, output_path, array_names, widths, heights, 
                         input_filename, is_sprite_sheet=False, sprite_cols=1, sprite_rows=1):
    """
    ...
    try:
        with open(output_path, 'w') as f:
            # Write header
            f.write(f"// SPDX-FileCopyrightText: {datetime.now().year} KOINSLOT, Inc.\n")
            ...
            if is_sprite_sheet:
                ...
                f.write(f" *   engine.display.drawBitmap(x, y, {base_name.upper()}_FRAME_WIDTH, \n")
                f.write(f" *                            {base_name.upper()}_FRAME_HEIGHT, {array_names[0]});\n")
                f.write(f" *   \n")
                f.write(f" *   // Animate through frames:\n")
                f.write(f" *   const uint8_t* frames[] = {{{', '.join(array_names)}}};\n")
                f.write(f" *   engine.display.drawBitmap(x, y, {base_name.upper()}_FRAME_WIDTH,\n")
                f.write(f" *                            {base_name.upper()}_FRAME_HEIGHT, frames[currentFrame]);\n")
            else:
                f.write(f" *   #include \"{os.path.basename(output_path)}\"\n")
                f.write(f" *   engine.display.drawBitmap(x, y, {array_names[0].upper()}_WIDTH, \n")
                f.write(f" *                            {array_names[0].upper()}_HEIGHT, {array_names[0]});\n")
            
            f.write(f" */\n\n")
            
            # Generate include guard
            guard_name = os.path.splitext(os.path.basename(output_path))[0].upper() + "_H"
            ...
            # Write arrays
            for i, (byte_array, array_name, width, height) in enumerate(zip(byte_arrays, array_names, widths, heights)):
                if is_sprite_sheet:
...
For sprite sheets:
  ...
  engine.display.drawBitmap(x, y, SPRITE_FRAME_WIDTH, SPRITE_FRAME_HEIGHT, frames[currentFrame]);
    ...
    parser.add_argument("array_name", nargs='?', help="C array name (default: derived from output filename)")
    
    # Image processing options
    parser.add_argument("--width", type=int, default=KYWY_WIDTH, 
                       help=f"Target width in pixels (default: {KYWY_WIDTH})")
    parser.add_argument("--height", type=int, default=KYWY_HEIGHT, 
                       help=f"Target height in pixels (default: {KYWY_HEIGHT})")
    parser.add_argument("--threshold", type=int, metavar="0-255",
    ...
    parser.add_argument("--sprite-cols", type=int, metavar="N",
                       help="Number of sprite columns (creates sprite sheet)")
    parser.add_argument("--sprite-rows", type=int, metavar="N",
    ...
    if is_sprite_sheet:
        if args.sprite_cols is None or args.sprite_rows is None:
            print("Error: Both --sprite-cols and --sprite-rows must be specified for sprite sheets")
            sys.exit(1)
    ...
    if is_sprite_sheet:
        ...
        img = load_and_prepare_image(args.input, args.width, args.height, 
                                   args.threshold, resize=False)
        ...
        if not args.no_resize:
            frame_width = args.width // args.sprite_cols if args.width else frames[0].size[0]
            frame_height = args.height // args.sprite_rows if args.height else frames[0].size[1]
            
            resized_frames = []
            for i, frame in enumerate(frames):
                if frame.size != (frame_width, frame_height):
                    print(f"Resizing frame {i} from {frame.size[0]}x{frame.size[1]} to {frame_width}x{frame_height}")
        ...
        generate_header_file(byte_arrays, args.output, array_names, widths, heights, 
                           input_filename, is_sprite_sheet=True, 
                           sprite_cols=args.sprite_cols, sprite_rows=args.sprite_rows)
        ...
        print(f'  engine.display.drawBitmap(x, y, {base_array_name.upper()}_FRAME_WIDTH,')
        print(f'                           {base_array_name.upper()}_FRAME_HEIGHT, frames[currentFrame]);')
        
    else:
        # Single image mode
        img = load_and_prepare_image(args.input, args.width, args.height, 
                                   args.threshold, resize=not args.no_resize)
        ...
        # Generate header file
        input_filename = os.path.basename(args.input)
        generate_header_file([byte_array], args.output, [base_array_name], 
                           [img.size[0]], [img.size[1]], input_filename)
        
        print("-" * 50)
        print("Conversion complete!")
        print(f"\nTo use in your Kywy project:")
        print(f'  #include "{os.path.basename(args.output)}"')
        print(f'  engine.display.drawBitmap(x, y, {base_array_name.upper()}_WIDTH, {base_array_name.upper()}_HEIGHT, {base_array_name});')
