...
const uint8_t spriteSheetData[] = {
    ...
    };
...
::SpriteSheet spriteSheet = ::SpriteSheet(
    spriteSheetData, spriteWidth *numFrames, spriteHeight, numFrames);
...
::Actor::Message asteroidCollisionMessage =
    ::Actor::Message(ASTEROID_COLLISION);
...
public:
  ...
  void initialize() {
    ship.setPosition(DISPLAY_WIDTH / 2, DISPLAY_HEIGHT / 2 - shipArea);
  }
  ...
  void handle(::Actor::Message *message) {
    ...
    case Kywy::Events::TICK:
      ...
      else if (xPosition > DISPLAY_WIDTH)
        xPosition = 0;

      if (yPosition < 0)
        yPosition = DISPLAY_HEIGHT;
      else if (yPosition > DISPLAY_HEIGHT)
        yPosition = 0;

      ship.setPosition((int)xPosition, (int)yPosition);
      ...
      engine.display.drawLine((int16_t)topX, (int16_t)topY, (int16_t)rightX,
                              (int16_t)rightY);
      engine.display.drawLine((int16_t)topX, (int16_t)topY, (int16_t)leftX,
                              (int16_t)leftY);
      engine.display.drawLine((int16_t)leftX, (int16_t)leftY,
                              (int16_t)xPosition, (int16_t)yPosition);
      engine.display.drawLine((int16_t)xPosition, (int16_t)yPosition,
                              (int16_t)rightX, (int16_t)rightY);

      // Draw tail
      trailX = xPosition - shipArea * cos(shipAngle * M_PI / 180.0);
      trailY = yPosition + shipArea * sin(shipAngle * M_PI / 180.0);

      if (buttonUpPressed) {
        engine.display.drawCircle(
            (int16_t)(xPosition -
                      0.5 * shipArea * cos(shipAngle * M_PI / 180.0)),
            (int16_t)(yPosition +
                      0.5 * shipArea * sin(shipAngle * M_PI / 180.0)),
            4);
        engine.display.drawCircle((int16_t)(trailX), (int16_t)(trailY), 3);
        engine.display.drawCircle(
            (int16_t)(xPosition -
                      1.5 * shipArea * cos(shipAngle * M_PI / 180.0)),
            (int16_t)(yPosition +
                      1.5 * shipArea * sin(shipAngle * M_PI / 180.0)),
            2);
      }
...
public:
  ...
  const char *getName() { return "bulletManager"; };
  ...
  void handle(::Actor::Message *message) {
    ...
    case Kywy::Events::BUTTON_RIGHT_PRESSED:
      for (int i = 0; i < maxBullets; i++) {
        ...
        bullets[i].yVel =
            yVelocity * -sin(shipManager.shipAngle * M_PI / 180.0);
        break;
      }
      break;
    ...
    case Kywy::Events::TICK:
      for (int i = 0; i < maxBullets; i++) {
        if (bullets[i].exist) {
          ...
          if (bullets[i].x < 0 || bullets[i].x > DISPLAY_WIDTH ||
              bullets[i].y < 0 || bullets[i].y > DISPLAY_HEIGHT) {
...
public:
  ...
  void initialize() {
    for (int i = 0; i < numAsteroids; i++) {
      asteroids[i] =
          Asteroid{.x = random(DISPLAY_WIDTH),
  ...
  void drawAsteroids() {
    for (int i = 0; i < numAsteroids; i++) {
      if (asteroids[i].exist) {
        engine.display.drawCircle((int)asteroids[i].x, (int)asteroids[i].y,
                                  (2 * asteroids[i].radius));
      }
    }
  }
  ...
  void handle(::Actor::Message *message) {
    ...
    case Kywy::Events::TICK:
      ...
      for (int i = 0; i < numAsteroids; i++) {
        ...
        if (asteroids[i].x < 0 || asteroids[i].x > DISPLAY_WIDTH ||
            asteroids[i].y < 0 || asteroids[i].y > DISPLAY_HEIGHT) {
      ...
      // Game End
      for (int i = 0; i < numAsteroids; i++) {
        if ((abs(asteroids[i].x - shipManager.xPosition) <
             2 * asteroids[i].radius) and
            (abs(asteroids[i].y - shipManager.yPosition) <
             2 * asteroids[i].radius)) {
...
public:
  ...
  void handle(::Actor::Message *message) {
    ...
    case START_SCREEN: {
      ...
      engine.display.drawBitmap(0, 0, 144, 168,
                                (uint8_t *)asteroidSplashScreen);
      engine.display.update();
    ...
    case Kywy::Events::TICK: {
      ...
      for (int i = 0; i < asteroidManager.numAsteroids; i++) {
        ...
        for (int j = 0; j < bulletManager.maxBullets; j++) {
          ...
          float dx =
              asteroidManager.asteroids[i].x - bulletManager.bullets[j].x;
          float dy =
              asteroidManager.asteroids[i].y - bulletManager.bullets[j].y;
