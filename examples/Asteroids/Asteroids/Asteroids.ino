...
public:
  ...
  void handle(::Actor::Message *message) {
    ...
    for (int i = 0; i < numAsteroids; i++) {
      ...
      if((asteroids[i].x-ship.x < asteroids[i].radius) or (asteroids[i].y-ship.y < asteroids[i].radius)){
...
public:
  ...
  void handle(::Actor::Message *message) {
    switch (message->signal) {
      ...
      case Kywy::Events::TICK:
      if (buttonPressed) {
        engine.display.fillCircle(ship.x,ship.y,10);
      }
      break;
    }
  }
