...
#include "player_sprite.hpp" // Player sprite data
...
// Player position variables, initialized to start position
...
// We take the x, y poisitions and the width and height of the area to check
...
void loop() {
  ...
  engine.display.drawBitmap(playerX, playerY, PLAYER_SPRITE_WIDTH, PLAYER_SPRITE_HEIGHT, player_sprite_data);

  // Check win condition and draw win screen if reached goal
  ...
  engine.display.update();
  delay(100); // Small delay for smooth movement, note that this is the frame time. It affects movement speed and input responsiveness. In later examples we discuss improved ways to handle game loops with the kywy engine.
}
